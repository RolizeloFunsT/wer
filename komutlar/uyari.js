...
if(!message.member.hasPermission("KICK_MEMBERS")) return message.channel.send(`Bu komutu kullanabilmek için \`Üyeleri At\` yetkisine sahip olmalısın.`);
  ...
  if (reason.length < 1) return message.reply('**Lütfen tüm bilgileri eksiksiz doldur,örnek e!uyar @kullanıcı#1234 Kural İhlali**');
  if (message.mentions.users.size < 1) return message.reply('**Lütfen tüm bilgileri eksiksiz doldur,örnek !!uyar @kullanıcı#1234 Kural İhlali**').catch(console.error);
